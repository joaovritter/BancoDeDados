...
WHERE FUNCIONARIO.Dnr = DEPARTAMENTO.Dnumero
AND DEPARTAMENTO.Dnome = "Pesquisa";
...
SELECT  F.Pnome AS NOME, F.Unome AS SOBRENOME, P.Projnome AS PROJETO, T.Horas AS HORAS
FROM FUNCIONARIO AS F, PROJETO AS P, TRABALHA_EM AS T
INNER JOIN T ON F.Cpf = T.Fcpf
INNER JOIN P ON T.Pnr = P.Projnumero
WHERE P.Projnome = 'ProdutoX';

SELECT Projnumero, Unome, Endereco, Datanasc, Dnumero
FROM FUNCIONARIO
INNER JOIN DEPARTAMENTO ON Cpf_gerente = Cpf
INNER JOIN PROJETO ON Dnumero = Dnum
